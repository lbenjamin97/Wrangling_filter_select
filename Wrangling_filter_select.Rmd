...
```
# Check column names and sample data (including year)
```{r}
colnames(movies)
head(movies$year, 20)  # shows first 20 year values

# Count how many unique years there are
length(unique(movies$year))
...
# Count how many movies were released in 1995
```{r}
movies %>%
  filter(year == 1995) %>%
  summarise(count = n())
```
it was 1 movie released in 1995 in the movies data.
...
```
these are the movies with the highest budgets
...
# Define the URL for the IMDb page of a Harry Potter movie
url <- "https://www.imdb.com/title/tt1201607/"  # Example: Harry Potter and the Deathly Hallows: Part 2

# Read the HTML content of the page
page <- read_html(url)

# Extract the IMDb rating and vote count
rating <- page %>%
...
cat("IMDb Rating:", rating, "\n")
cat("Vote Count:", votes, "\n")
```
```{r}
library(dplyr)
library(stringr)
